...
! PRINT_WARNING
  ...
  !*****************************************************************************************************************
   ...
   function gas_t_str(acc_rate,h,ngc)  
    ...
    integer(kind=4),intent(out),optional :: ngc         ! number of giant cloud formed
    ...
    ! Accretion rate
    !
    log_acc_rate = gas_mass(acc_rate)*1.d2   ! convert from 10^11Msun/Gyr in Msun/yr
    if (log_acc_rate .gt. 0.d0) then
        ...
        if (h .gt. 0.d0) then
            logh = log10(h)
            ! load log(t_str)
            gas_t_str = locate2D(log_acc_rate,logh,t_str, &  
                            nAccRateBins,nDiscScaleBins, &
                            AccRatemin,DiscScalemin, &
                            dlAccRate,dlDiscScale)                  
            gas_t_str = (1.d1)**gas_t_str ! Here the value is given in Gyr
            !
            if (present(ngc)) then
                ngc = int(locate2D(log_acc_rate,logh,ngc_table, &  
                            nAccRateBins,nDiscScaleBins, &
                            AccRatemin,DiscScalemin, &
                            dlAccRate,dlDiscScale),4)
            end if
        end if
    end if
